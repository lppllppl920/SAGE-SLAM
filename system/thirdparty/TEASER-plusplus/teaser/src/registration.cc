...
void teaser::TLSScaleSolver::solveForScale(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst,
    const Eigen::Matrix<double, 1, Eigen::Dynamic>& noise_bounds, double* scale,
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {
  ...
  Eigen::Matrix<double, 1, Eigen::Dynamic> raw_scales = v2_dist.array() / v1_dist.array();
  auto beta = noise_bounds * sqrt(cbar2_);
  Eigen::Matrix<double, 1, Eigen::Dynamic> alphas = beta.array() * v1_dist.cwiseInverse().array();
...
void teaser::TLSTranslationSolver::solveForTranslation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst,
    const Eigen::Matrix<double, 1, Eigen::Dynamic>& noise_bounds, Eigen::Vector3d* translation,
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {
  assert(src.cols() == dst.cols());
  if (inliers) {
    assert(inliers->cols() == src.cols());
  ...
  Eigen::Matrix<double, 1, Eigen::Dynamic> alphas =
      beta.array() * Eigen::MatrixXd::Ones(1, N).array();
...
void teaser::RobustRegistrationSolver::computeTIMs(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& v,
    const Eigen::Matrix<double, 1, Eigen::Dynamic>& noise_bounds,
    Eigen::Matrix<double, 3, Eigen::Dynamic>& vtilde,
    Eigen::Matrix<double, 1, Eigen::Dynamic>& noise_bounds_tilde,
    Eigen::Matrix<int, 2, Eigen::Dynamic>* map) {

  auto N = v.cols();
  vtilde.resize(3, N * (N - 1) / 2);
  noise_bounds_tilde.resize(1, N * (N - 1) / 2);
  map->resize(2, N * (N - 1) / 2);

#pragma omp parallel for default(none) shared(N, v, noise_bounds, vtilde, noise_bounds_tilde, map)
  for (size_t i = 0; i < N - 1; i++) {
    ...
    Eigen::Matrix<double, 1, 1> m_noise = noise_bounds.col(i);

    Eigen::Matrix<double, 3, Eigen::Dynamic> temp = v - m * Eigen::MatrixXd::Ones(1, N);
    Eigen::Matrix<double, 1, Eigen::Dynamic> temp_noise =
        noise_bounds + m_noise * Eigen::MatrixXd::Ones(1, N);

    // concatenate to the end of the tilde vector
    vtilde.middleCols(segment_start_idx, segment_cols) = temp.rightCols(segment_cols);
    noise_bounds_tilde.middleCols(segment_start_idx, segment_cols) =
        temp_noise.rightCols(segment_cols);
...
teaser::RegistrationSolution teaser::RobustRegistrationSolver::solve(
  ...
  Eigen::Matrix<double, 1, Eigen::Dynamic> noise_bounds_tims;
  src_tims_ = computeTIMs(src, &src_tims_map_);
  computeTIMs(dst, noise_bounds, dst_tims_, noise_bounds_tims, &dst_tims_map_);

  TEASER_DEBUG_INFO_MSG("Starting scale solver.");
  scale_inliers_mask_.resize(1, src_tims_.cols());
  scale_solver_->solveForScale(src_tims_, dst_tims_, noise_bounds_tims, &(solution_.scale),
                               &scale_inliers_mask_);
  ...
  Eigen::Matrix<double, 1, Eigen::Dynamic> pruned_noise_bounds_tims(1, max_clique_.size());
  ...
  if (params_.rotation_tim_graph == INLIER_GRAPH_FORMULATION::CHAIN) {
    ...
    for (size_t i = 0; i < max_clique_.size(); ++i) {
      ...
      pruned_noise_bounds_tims.col(i) = noise_bounds.col(leaf) + noise_bounds.col(root);
    ...
    Eigen::Matrix<double, 1, Eigen::Dynamic> noise_bounds_inliers(1, max_clique_.size());

    for (size_t i = 0; i < max_clique_.size(); ++i) {
      src_inliers.col(i) = src.col(max_clique_[i]);
      dst_inliers.col(i) = dst.col(max_clique_[i]);
      noise_bounds_inliers.col(i) = noise_bounds.col(max_clique_[i]);
    }
    // construct the TIMs
    pruned_src_tims_ = computeTIMs(src_inliers, &src_tims_map_rotation_);
    computeTIMs(dst_inliers, noise_bounds_inliers, pruned_dst_tims_, pruned_noise_bounds_tims,
                &dst_tims_map_rotation_);
  }

  // Remove scaling for rotation estimation
  pruned_dst_tims_ *= (1 / solution_.scale);
  pruned_noise_bounds_tims = pruned_noise_bounds_tims * (1 / solution_.scale);
  ...
  rotation_inliers_mask_.resize(1, pruned_src_tims_.cols());
  rotation_solver_->solveForRotation(pruned_src_tims_, pruned_dst_tims_, pruned_noise_bounds_tims,
                                     &(solution_.rotation), &rotation_inliers_mask_);
  ...
  Eigen::Matrix<double, 1, Eigen::Dynamic> rotation_pruned_noise_bounds(1, max_clique_.size());
  for (size_t i = 0; i < max_clique_.size(); ++i) {
    rotation_pruned_src.col(i) = src.col(max_clique_[i]);
    rotation_pruned_dst.col(i) = dst.col(max_clique_[i]);
    rotation_pruned_noise_bounds.col(i) = noise_bounds.col(max_clique_[i]);
  }

  // Solve for translation
  TEASER_DEBUG_INFO_MSG("Starting translation solver.");
  translation_inliers_mask_.resize(1, rotation_pruned_src.cols());
  translation_solver_->solveForTranslation(
      solution_.scale * solution_.rotation * rotation_pruned_src, rotation_pruned_dst,
      rotation_pruned_noise_bounds, &(solution_.translation), &translation_inliers_mask_);
...
void teaser::GNCTLSRotationSolver::solveForRotation(
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
    const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst,
    const Eigen::Matrix<double, 1, Eigen::Dynamic>& noise_bounds, Eigen::Matrix3d* rotation,
    Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) {
  assert(rotation);                 // make sure R is not a nullptr
  assert(src.cols() == dst.cols()); // check dimensions of input data
  assert(params_.gnc_factor > 1);   // make sure mu will increase
  ...
  auto noise_bounds_sq = noise_bounds.array().square();
  double noise_bound_sq = std::pow(params_.noise_bound, 2);
  if (noise_bound_sq < 1e-16) {
    noise_bound_sq = 1e-2;
  }

  TEASER_DEBUG_INFO_MSG("GNC rotation estimation noise bound:" << noise_bound);
  TEASER_DEBUG_INFO_MSG("GNC rotation estimation noise bound squared:" << noise_bound_sq);

  TEASER_DEBUG_INFO_MSG("GNC rotation estimation noise bounds:" << noise_bounds);
  TEASER_DEBUG_INFO_MSG("GNC rotation estimation noise bounds squared:" << noise_bounds_sq);
  ...
  for (size_t i = 0; i < params_.max_iterations; ++i) {
    ...
    *rotation = teaser::utils::svdRot(src, dst, weights);
    ...
    for (size_t j = 0; j < match_size; ++j) {
      // Also calculate cost in this loop
      // Note: the cost calculated is using the previously solved weights
      th1 = (mu + 1) / mu * noise_bounds_sq(0, j);
      th2 = mu / (mu + 1) * noise_bounds_sq(0, j);
      ...
      if (residuals_sq(j) >= th1) {
        ...
        weights(j) = sqrt(noise_bounds_sq(0, j) * mu * (mu + 1) / residuals_sq(j)) - mu;
        assert(weights(j) >= 0 && weights(j) <= 1);
      }
    }
