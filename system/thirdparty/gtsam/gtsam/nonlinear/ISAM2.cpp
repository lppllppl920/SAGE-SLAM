...
namespace gtsam
{
                                     ...
                                     ISAM2Result *result)
  {
    ...
    if (debug)
      cachedBoundary.print("Boundary factors: ");
                           ...
                           ISAM2Result::DetailedResults *detail)
  {
    ...
    if (ISDEBUG("ISAM2 AddVariables"))
      newTheta.print("The new variables are: ");
  ...
  /* ************************************************************************* */
  void ISAM2::updateVariables(const Values &updateTheta,
                              ISAM2Result::DetailedResults *detail)
  {
    gttic(addNewVariables);

    theta_.update(updateTheta);
    if (ISDEBUG("ISAM2 UpdateVariables"))
      updateTheta.print("The updated variables are: ");
    // Add zeros into the VectorValues
    delta_.update(updateTheta.zeroVectors());
    deltaNewton_.update(updateTheta.zeroVectors());
    RgProd_.update(updateTheta.zeroVectors());
    deltaReplacedMask_.clear();
    // deltaReplacedMask_.insert(updateTheta.keys().begin(), updateTheta.keys().end());

    // // New keys for detailed results
    // if (detail && params_.enableDetailedResults)
    // {
    //   for (Key key : updateTheta.keys())
    //   {
    //     detail->variableStatus[key].isNew = true;
    //   }
    // }
      ...
      bool force_relinearize)
  {
    ...
    return update(newFactors, newTheta, updateTheta, params);
  }
                            ...
                            const ISAM2UpdateParams &updateParams)
  {
    ...
    // \Theta:=\Theta\cup\Theta_{new}.
    addVariables(newTheta, result.details());
    // Manually update values of some variables
    updateVariables(updateTheta, result.details());
