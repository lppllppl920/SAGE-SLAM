  ...
  FSurf64::TDescriptor &mean)
{
  ...
  // TOOD: XT we should mimic this way of iterating through the std::vector. In this way we don't need to create temporary storage.
...
{
  double sqd = 0.;
  // This unroll is for speeding up to reduce the overall overhead of loop iteration
  for(int i = 0; i < FSurf64::L; i += 4)
  {
    sqd += (a[i  ] - b[i  ])*(a[i  ] - b[i  ]);
