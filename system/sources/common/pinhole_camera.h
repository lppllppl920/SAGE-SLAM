...
public:
  ...
  inline EIGEN_DEVICE_FUNC bool PixelValid(T x, T y, long border = 0) const;

  /**
   * Check if pixel is inside camera
   */
  inline EIGEN_DEVICE_FUNC bool PixelValid(const PixelTRef& pixel, long border = 0) const;
  ...
  template <typename T>
  inline PinholeCamera<Scalar> ComputeResizedCam(const T &new_width, const T &new_height);
