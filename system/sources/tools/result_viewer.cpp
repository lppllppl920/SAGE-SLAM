...
std::vector<SE3f> LoadTrajectoryTUM(std::string path)
{
  ...
  if (not f)
  {
    LOG(WARNING) << "[LoadTrajectoryTUM] Failed to open trajectory file: '" << path << "'";
...
int main(int argc, char *argv[])
{
  ...
  {
    LOG(INFO) << "[result_viewer] Loading estimated trajectory to display";
    est_poses = LoadTrajectoryTUM(FLAGS_trajectory_file);
  }

  if (not FLAGS_save_gt.empty())
  {
    LOG(INFO) << "[result_viewer] Saving ground truth trajectory to: " << FLAGS_save_gt;
    SaveTrajectoryTUM(FLAGS_save_gt, gt_poses);
  }

  if (FLAGS_no_display)
  {
    LOG(INFO) << "[result_viewer] Exiting as requested with --no_display";
    exit(0);
  }
  ...
  {
    LOG(INFO) << "[result_viewer] Loading entire dataset...";
    auto frames = data_interface->GetAll();

    LOG(INFO) << "[result_viewer] Processing the dataset...";
    for (uint i = 0; i < frames.size(); i += FLAGS_depth_skip)
    {
      ...
      cv::Mat dpt(frame.dpt.rows, frame.dpt.cols, CV_32FC1);
      frame.dpt.convertTo(dpt, CV_32FC1);
      dd.dpt = torch::from_blob(static_cast<float *>(dpt.ptr<float>(0)), {frame.dpt.rows, frame.dpt.cols},
